...
- (void)transitionToMainInterface {
    ...
    [[AdMobManager sharedManager] reloadAdConfigWithSuccess:^(NSArray<AdMobConfigModel *> *configs) {
        NSLog(@"[AccountLoginViewController] 登录后刷新广告配置成功，共%lu个配置", (unsigned long)configs.count);
    } failure:^(NSError *error) {
        NSLog(@"[AccountLoginViewController] 登录后刷新广告配置失败: %@", error.localizedDescription);
    }];
}
