...
+ (NSArray<RechargeRecordModel *> *)modelsFromResponse:(NSArray *)array {
    ...
    for (NSDictionary *dict in array) {
        if ([dict isKindOfClass:[NSDictionary class]]) {
            ...
            // 对齐安卓：时间格式化处理（使用 addTime 时间戳）
            if (model.addTime > 0) {
                NSDate *date = [NSDate dateWithTimeIntervalSince1970:model.addTime / 1000.0]; // 安卓时间戳可能是毫秒
...
// 分页相关（对齐安卓：使用index和count）
@property (nonatomic, assign) NSInteger index; // 分页起始位置，从0开始
@property (nonatomic, assign) NSInteger count; // 每页条数，对齐安卓：10
...
- (void)viewDidLoad {
    ...
    // 初始化数据（对齐安卓：使用index和count）
    self.records = [NSMutableArray array];
    self.index = 0;
    self.count = 10; // 对齐安卓：每页10条
...
- (void)loadRechargeHistory:(BOOL)isRefresh {
    ...
    self.isLoading = YES;
    if (isRefresh) {
        self.index = 0; // 对齐安卓：刷新时重置index为0
        self.hasMoreData = YES;
    }
    
    // 对齐安卓：分页参数（index和count）
    ...
    // 对齐安卓：使用钱包明细列表接口（user/getUserBudgetList），POST请求
    [[NetworkManager sharedManager] POST:BUNNYX_API_USER_BUDGET_LIST
        ...
        // 对齐安卓：处理返回数据格式 { "code": 0, "data": [...], "promptType": "..." }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    containerView.layer.cornerRadius = 12; // 对齐安卓：圆角12dp
    [cell.contentView addSubview:containerView];
    
    [containerView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(cell.contentView).insets(UIEdgeInsetsMake(0, 20, 0, 20)); // 对齐安卓：paddingHorizontal 20dp
        make.top.bottom.equalTo(cell.contentView).insets(UIEdgeInsetsMake(0, 0, 15, 0)); // 对齐安卓：marginBottom 15dp
        make.height.mas_equalTo(80); // 对齐安卓：itemHeight 80dp
    }];
    containerView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.03];
    
    // 对齐安卓：金币图标（icon_mine_coin_default）
    ...
    [coinIcon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(containerView).offset(20); // 对齐安卓：marginLeft 20dp
        make.centerY.equalTo(containerView);
        make.width.height.mas_equalTo(40); // 对齐安卓：40dp × 40dp
    }];
    
    // 对齐安卓：金币数量（绿色，20sp bold）
    ...
    [coinsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(coinIcon.mas_right).offset(15); // 对齐安卓：间距15dp
        make.top.equalTo(containerView).offset(15); // 对齐安卓：marginTop 15dp
    }];
    ...
    [timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(coinsLabel.mas_bottom).offset(8); // 对齐安卓：间距8dp
    }];
    ...
    [amountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(containerView).offset(-20); // 对齐安卓：marginRight 20dp
        make.centerY.equalTo(containerView);
    }];
    
    // 对齐安卓：状态标签（根据state显示不同文字和颜色）
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return 95; // 对齐安卓：80dp + 15dp marginBottom
}
